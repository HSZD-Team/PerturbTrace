...
Solver-facing strategy skill for PerturbTrace decoupled harness (`init-run --skill`).

This is **not** the `/bda` monitor skill. Monitor skill lives at `skills/bda/`.

## Verify locally

From `PerturbTrace/benchmarks/perturbtrace`:

```bash
PYTHONPATH=. python -m PerturbTrace.baselines.harness.cli init-run \
