# Decoupled BDAbench Harness

This folder is the canonical handoff surface for running Codex/LLM evaluations against BDAbench hidden-oracle tasks.
...
```powershell
python -m BDAbench.baselines.harness.cli <command> ...
```
...
- `BDAbench.baselines.framework.profile`: task manifest loader.
- `BDAbench.baselines.framework.types`: shared dataclasses.
- `BDAbench.baselines.framework.actions`: solver output parser and batch validator.
- `BDAbench.baselines.framework.oracle`: hidden oracle scoring.
- `BDAbench.baselines.framework.metrics`: metric computation.
- `BDAbench.baselines.framework.leakage`: prompt/trace leakage audit.
- `BDAbench.baselines.framework.trace`: low-level trace writer.
...
```text
BDAbench/baselines/harness_runs
```
...
```powershell
$runRoot = python -m BDAbench.baselines.harness.cli init-run `
  --run-id example_run `
  --strategy-version example_strategy `
  --task-profile BDAbench/tasks/c3_gdsc_drug_response_v0/task_manifest.yaml `
  --skill BDAbench/skills/restricted-clean-perturbation-strategy
