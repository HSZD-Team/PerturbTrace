...
This file is the monitor-facing entrypoint for launching BDAbench Codex/LLM baseline evaluations.
...
```text
BDAbench/baselines/harness/README.md
```
...
task_index: BDAbench/tasks/task_index.yaml
task_profiles:
  - BDAbench/tasks/c3_gdsc_drug_response_v0/task_manifest.yaml
skill: <external_solver_skill_dir>
strategy_version: restricted_clean_v1
output_dir: BDAbench/baselines/harness_runs/restricted_clean_v1
...
```text
BDAbench/baselines/harness/artifacts.py
```
...
- edit `BDAbench/baselines/harness/` during an eval run;
...
```text
BDAbench/baselines/harness_runs/xhigh_full_tasks_20260615_direct_prompt/direct_prompt_orchestrator.py
```
...
```text
BDAbench/baselines/goal_memory/<goal_id>/
```
